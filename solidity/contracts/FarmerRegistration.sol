...
contract FarmerRegistration {
   ...
   //emitted whenever a farmer is registered
    ...
    event OrderPlaced(uint orderId, address buyer, uint batchId, uint quantity, uint totalPrice);
    ...
    function placeOrder(uint _batchId, uint _quantity, uint _pricePerLiter) public payable {
        require(_quantity > 0, "Quantity must be greater than zero");
        uint totalPrice = _quantity * _pricePerLiter;
        require(msg.value == totalPrice, "Incorrect payment amount");

        orderCounter++;
        orders[orderCounter] = Order(orderCounter, msg.sender, _batchId, _quantity, totalPrice, false);

        emit OrderPlaced(orderCounter, msg.sender, _batchId, _quantity, totalPrice);
    }
